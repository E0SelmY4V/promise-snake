...
type Last<T, S = never> = T extends readonly [...any[], infer N] ? N : T extends AnyT<never> ? S : Ifer<T, S>;
...
type LRtn<S extends AnyT<Part<AnyF>>, T = void> = ReturnType<Sure<Last<S, 0>, AnyF, () => T>>;
...
declare global {
	interface Promise<T> {
		 ...
		 * resolved.
		 */
		thens<K extends AnyT<Part<AnyF<[value: any]>>>>(onfulfilleds: readonly [...K]): Promise<Awaited<LRtn<K, T>>>;
	}
	interface PromiseConstructor {
		 ...
		 */
		thens<K extends AnyT<Part<AnyF<[value: any]>>>>(onfulfilleds: readonly [...K]): Promise<Awaited<LRtn<K>>>;
	}
	namespace Promise {
		/**
